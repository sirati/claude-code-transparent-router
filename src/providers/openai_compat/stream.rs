...
impl Translator {
    ...
    fn on_tool_call(&mut self, call: &Value, out: &mut String) {
        ...
        if announces_new && !same_call {
            ...
            out.push_str(&sse::content_block_start(
                index,
                sse::tool_use_block(id, name),
            ));
    ...
    fn ensure(&mut self, kind: Kind, out: &mut String) -> usize {
        match (&self.open, kind) {
            ...
            _ => {
                ...
                let index = self.next_index;
                self.next_index += 1;
                out.push_str(&sse::content_block_start(index, sse::text_block()));
